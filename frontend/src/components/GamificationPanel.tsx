  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
      ...
      <div className="flex items-center gap-4 mb-4">
        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center flex-shrink-0 shadow-lg">
          <span className="text-xl font-bold text-white">{level}</span>
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <Trophy className="w-4 h-4 text-yellow-500" />
            <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Nível {level} — {title}</h3>
          </div>
          <div className="mt-2">
            ...
            <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-yellow-400 to-yellow-500 rounded-full transition-all" style={{ width: `${progress}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1 mb-3">
        {unlocked.slice(0, 8).map(ach => (
          <div
            key={ach.slug}
            className="w-7 h-7 rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center"
      ...
      {/* Achievements list */}
      {showAchievements && (
        <div className="mt-3 space-y-1 max-h-60 overflow-y-auto border-t border-gray-100 dark:border-gray-700 pt-3">
          {unlocked.map(ach => (
            <div key={ach.slug} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-yellow-50 dark:bg-yellow-900/10 text-sm">
              <Award className="w-4 h-4 text-yellow-500 flex-shrink-0" />
              <span className="flex-1 text-gray-700 dark:text-gray-300">{ach.title}</span>
              <span className="text-xs text-yellow-600">+{ach.xpReward} XP</span>
            </div>
          ))}
          {locked.slice(0, 5).map(ach => (
            <div key={ach.slug} className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm opacity-50">
          ...
          {locked.length > 5 && (
            <p className="text-xs text-center text-gray-400 py-1">+{locked.length - 5} bloqueadas</p>
          )}
        </div>
      )}
      ...
      {/* Counters */}
      {showCounters && (
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-gray-100 dark:border-gray-700 pt-3">
...
function CounterRow({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex justify-between items-center text-xs px-2 py-1 bg-gray-50 dark:bg-gray-700/50 rounded">
